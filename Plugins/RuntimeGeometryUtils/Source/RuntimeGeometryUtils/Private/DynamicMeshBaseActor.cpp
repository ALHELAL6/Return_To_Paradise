...
void ADynamicMeshBaseActor::ExportMesh(ADynamicMeshBaseActor* OtherMeshActor, FString Path, bool ReverseOrientation)
{
	...
	FDynamicMesh3 Mesh;
	OtherMeshActor->GetMeshCopy(Mesh);

	RTGUtils::WriteOBJMesh(Path, Mesh, ReverseOrientation);
...
void ADynamicMeshBaseActor::CleanMesh(ADynamicMeshBaseActor* OtherMeshActor)
{
	...
	FDynamicMesh3 CompactMesh;
	CompactMesh.CompactCopy(SourceMesh, false, false, false, false);
	CompactMesh.EnableTriangleGroups();
	FDynamicMeshAABBTree3 AABBTree(&CompactMesh, true);
	TFastWindingTree<FDynamicMesh3> Winding(&AABBTree, true);
	//OtherMeshActor->GetMeshCopy(CompactMesh);

	RecomputeNormals(CompactMesh);

	EditMesh([&](FDynamicMesh3& MeshToUpdate)
		{
			MeshToUpdate.CompactCopy(CompactMesh);
		});
}
